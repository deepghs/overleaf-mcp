...
Tools (27): `ping`, `list_servers`, `list_projects`, `open_project`, `list_files`, `read_file`, `edit_file`, `find_and_replace`, `compile`, `read_log`, `list_comments`, `read_comment_thread`, `add_comment`, `reply_comment`, `resolve_comment`, `reopen_comment`, `list_tracked_changes`, `accept_changes`, `reject_changes`, `download_file`, `download_project`, `download_output`, `create_file`, `create_folder`, `upload_file`, `rename_entity`, `delete_entity`.

This is the deepghs fork of netique/overleaf-mcp. On top of upstream it adds file management + downloads (`src/tools/fileManagement.ts`), anchored comments (`src/tools/addComment.ts`), headless password login (`src/auth/passwordLogin.ts`), multi-server support (`src/session/servers.ts`, `src/tools/listServers.ts`) and broadcast-driven file-tree sync (`src/session/activeProject.ts`).
...
- **Everything is keyed by server (`baseUrl`).** `getIdentity(baseUrl)`, `olGet/olPostJson/olDelete(baseUrl, …)`, `withAuthRetry(baseUrl, …)` and `ensureSocketForProject(baseUrl, projectId)` all take the server explicitly; project-scoped tools pass `ap.baseUrl`. Known servers = `OL_BASE_URL` (default) ∪ `OL_SERVERS` ∪ every host in `cookie.json` (`src/session/servers.ts`); `resolveServer()` maps a host or URL onto the stored origin (so `localhost:3000` finds `http://localhost:3000`). `list_projects` records `project_id → baseUrl` so `open_project` can omit `server`; with several servers logged in and no hint it refuses rather than guesses. Auth recovery for one server must not touch another's socket — hence `disconnectActiveIf(baseUrl)`. `list_projects` aggregates servers *sequentially* because an expired cookie triggers the Chrome login flow and two at once would race on the profile dir.

- **The file tree is kept current from server broadcasts, not by re-joining.** Modern real-time has no explicit `joinProject`; the only way to re-fetch the tree is to tear the socket down. So `activeProject.ts` applies `reciveNewDoc` / `reciveNewFile` / `reciveNewFolder` / `removeEntity` / `reciveEntityRename` / `reciveEntityMove` (arg order verified against `services/web/.../EditorController.mjs`; pure logic in `applyTreeEvent`, `src/api/projectTypes.ts`) to `project.rootFolder` and re-flattens `entities`. File-management tools `waitForTree()` for their own broadcast (`tree_sync: "event"`) and only fall back to `refreshTree()` (= close + open) after 5 s (`tree_sync: "reconnect"`). The doc cache survives tree events — ids don't change on rename/move — and only a deleted doc is dropped. Don't reintroduce close()+open() in the hot path; it used to cost two full reconnects per call.

- **`connectionRejected` on connect is classified, not swallowed.** Real-time emits it and disconnects when the `?projectId=` auto-join fails. Only `invalid session` becomes an `OverleafAuthError` (evicts the cookie); a bad or foreign project id is an `OverleafApiError`, so naming the wrong server never logs you out.
...
Cookie capture is via a dedicated headless-ish Chrome profile, driven over the Chrome DevTools Protocol — `node dist/index.js login` opens a window pointing at `<OL_BASE_URL>/project`, user logs in normally (captcha / Google OAuth / ORCID / institutional SSO / 2FA all work because it's a real Chrome), cookie is read via `Network.getCookies` once the dashboard loads, persisted to `<configDir>/overleaf-mcp/cookie.json` (mode 0600). The same flow auto-triggers when a tool call hits a 302→/login or 401/403. Dedicated profile means we never touch the user's real Chrome and never trigger a macOS Keychain prompt for it. The session cookie's *name* is a deployment setting — `overleaf_session2` on overleaf.com, `overleaf.sid` on CE / Server Pro (older installs `sharelatex.sid`) — so browser login must not insist on one name: it passes every cookie for the origin through and lets `validateCookie` (the `ol-user_id` meta on `/project`) decide. Upstream hard-coded `overleaf_session2` and therefore rejected successful logins on self-hosted instances. `login | logout | status --server <host|url>` target one server; `status` alone reports every known server. `OL_BASE_URL` (the default server) defaults to `https://www.overleaf.com`; `OL_SERVERS` pre-declares extra servers; `OL_HEADLESS=1` disables browser login; `OL_BROWSER` overrides the Chrome binary path; `OL_INSECURE=1` adds `--ignore-certificate-errors` for self-hosted CE with self-signed certs; `OL_CSRF` is optional (auto-discovered from `/project` HTML's `ol-csrfToken` meta).
...
- Don't refresh the file tree by reconnecting in the file-management hot path; rely on the broadcasts and keep the reconnect as the 5 s fallback only.
